...
def get_next_filename(folder="outputs"):
    ...
    # Find all existing png files and determine next number
...
# Function to open the outputs folder
...
def create_demo(
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            ...
            # Modified to include model selection and image saving
            def gradio_generate_wrapper(
                ...
                # Replace direct update calls with returning the update values
                model_loading_info.visible = True
                model_loading_info.value = f"Loading model {model_type}..."
                
                # Check if pipeline exists and has different offload setting
                if pipelines[model_type] is not None and pipelines[model_type].offload != offload_enabled:
                    # Recreate pipeline with new offload setting
                    pipelines[model_type] = None
                
                # Get or create pipeline with current offload setting
                ...
                # Save the image to outputs folder with a unique name
            ...
            # Add functionality to open the outputs folder
